...
sudokuGrid = new_puzzle.grid

# Convert the grid into a format suitable for Three.js
# For example, you can create a 2D array where each element
# is a path to the texture image or a flag indicating an input box is needed
threeJsGrid = []
for row in sudokuGrid:
    threeJsRow = []
    for number in row:
        if number == 0:
            threeJsRow.append("input")  # Flag for input box
        else:
            texturePath = "number_textures/{number}.png"
            threeJsRow.append(texturePath)  # Path to texture image
    threeJsGrid.append(threeJsRow)
